...
bool Maze::updatePlayer(char dir)
{
    ...
    if ( m_dir_player == TOP && isSquareWall(m_pos_player-getCol())==0  )   //Si le joueur va vers le haut
    {

        b=m_pos_player-getCol();
        if(isSquareBox(m_pos_player-getCol())==1 && _canPushBox(b,dir,a)==1)            //si il y a une brique poussable
        {
    ...
    if ( m_dir_player == BOTTOM && (isSquareWall(m_pos_player+getCol())==0 ))  //Vers le Bas
    {

        if(isSquareBox(m_pos_player+getCol())==1 && _canPushBox(m_pos_player+getCol(),dir,a)==1)
        {
...
void Maze::change_etat_jeu(std::vector<unsigned char> vec, std::vector<bool> a)
{
    m_field=vec;
    int b=0;
    unsigned short p=0;
...
/*
void Maze::affichefield(std::vector<unsigned char> vec)
{
    char u;
    for(int i=0; i<vec.size(); i++)
    {

        if(isSquareBox(vec[i]))
            std::cout<<"2";
        if(isSquareBoxPlaced(vec[i]))
            std::cout<<"3";
        if(isSquareWall(vec[i]))
            std::cout<<"1";
        if(isSquareGround(vec[i]))
            std::cout<<"0";
        if(vec[i]==5)
            std::cout<<"5";
        if(isSquareGoal(vec[i]))
            std::cout<<"4";
