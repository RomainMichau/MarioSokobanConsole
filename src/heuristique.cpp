...
unsigned short heuristique::heuristiquenaif(Maze m){
std::vector<unsigned short> box;
std::vector<unsigned short> goal=m.getGoals();
std::vector<std::vector<unsigned short>> plus_court;
std::vector<unsigned short> vecbox;
std::vector<unsigned short> vecgoal;
std::vector<unsigned short> vec;

unsigned short x,y,goal_reserve=0;
util u;
box=m.getPosBoxes();

for(unsigned int i=0; i<box.size(); i++)
{
    vecbox=u.doubldim(box[i],m);
    y=1000;
    for(unsigned int a=0; a<goal.size(); a++)
    {
        if(goal[a]!=0)
        {
            ...
            vecgoal=u.doubldim(goal[a],m);
            x=(fabs(vecbox[0]-vecgoal[0])+fabs(vecbox[1]-vecgoal[1]));
            if(x<y)
            {
                y=x;
                goal_reserve=a;
